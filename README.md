...
其他显示与数据源偏好（结算货币、结算时区、刷新间隔、汇率 Provider、主题、背景图、涨跌配色、美股盘前 / 盘后盈亏计入等）在后台“显示设置”中维护并持久化到数据库。美股盘前 / 盘后盈亏计入默认关闭；开启后影响美股今日盈亏和实时历史点，不改变市场状态展示。A 股 / 港股在盘前、盘中、盘后阶段默认使用当日最新行情。
