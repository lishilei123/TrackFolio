...
- **行情 Provider**：股票、基金行情通过可替换 Provider Adapter 接入，运行时按市场和资产类型自动选择数据源。
- **后台设置**：单实例后台密码保护，支持验证码、错误锁定、显示设置、主题设置、背景图、涨跌配色、行情校验、历史重算、汇率刷新和后台密码修改。
...
已实现的核心能力包括：持仓和交易维护、后台密码保护、实时行情 / 基金净值刷新、多币种汇总、历史盈亏快照、主题与显示设置、移动端基础适配。

以下能力仍属于后续迭代或部分实现：

- 独立资产详情页和单资产完整历史趋势页。
- 持仓列表列显隐、CSV 导出、数据备份与恢复。
- 按市场 / 类型拆分的完整历史贡献图。
- 分红、拆股、配股、税费等复杂交易事件。
...
- `routes/*`：API 路由，包括资产、持仓、交易、历史、设置、后台认证。
