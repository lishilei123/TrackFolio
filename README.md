...
| 命令 | 说明 |
|---|---|
| `npm run dev` | 构建前端并启动单体开发服务 |
| `npm test` | 运行后端单元测试 |
| `npm run build` | 构建 `app/web/dist` 和 `app/server/dist` |
| `npm start` | 启动已构建的单体应用 |

## 配置

所有运行时配置均通过环境变量读取。完整示例见 [.env.example](./.env.example)。
...
推荐用 Nginx / Caddy / Traefik 将整个站点反向代理到 Fastify，不要把静态站点和 `/api` 拆成两个域名或两个站点。
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `TRACKFOLIO_IMAGE` | `trackfolio:local` | Compose 构建或使用的镜像名 |
| `TRACKFOLIO_HTTP_PORT` | `8080` | 主机访问端口 |
| `LOG_LEVEL` | `info` | 后端日志级别 |
| `TRACKFOLIO_PROVIDER` | `auto` | 行情源 |
| `TRACKFOLIO_POSTGRES_PASSWORD` | `change-me` | PostgreSQL 模式数据库密码 |

### PostgreSQL 模式

叠加 `compose.postgres.yaml`：
...
- 服务端不维护独立域名白名单；反向代理需要保留 `Host`、`X-Forwarded-Host`、`X-Forwarded-Proto`。
- 公网部署必须优先使用 HTTPS。
