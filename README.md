...
- **历史分析**：DailyPnL 每日快照落库，首页走势图支持近 7 / 30 / 90 天和今年四个预设区间，并按区间自动选择日 / 周聚合。
- **多币种折算**：支持 CNY / USD / HKD 统一结算货币；汇率可按 Provider 自动获取或手动刷新，汇率异常时给出缺口提示。
- **行情 Provider**：股票、基金行情通过可替换 Provider Adapter 接入，运行时按市场和资产类型自动选择数据源；美股盘前 / 盘后盈亏计入口径可独立开启，默认关闭，A 股 / 港股默认使用当日最新行情。
- **后台设置**：单实例后台密码保护，解锁后按资产配置、已实现盈亏、显示设置、安全分区管理；支持验证码、错误锁定、资产删除、显示设置、主题设置、背景图、涨跌配色、美股盘前 / 盘后盈亏计入开关、行情校验、历史重算、汇率刷新和后台密码修改。
...
已实现的核心能力包括：持仓和交易维护、后台密码保护、资产配置 JSON 导入导出、实时行情 / 基金净值刷新、多币种汇总、历史盈亏快照、持仓占比环形图、主题与显示设置、美股盘前 / 盘后盈亏计入口径控制、移动端基础适配。
...
其他显示与数据源偏好（结算货币、结算时区、刷新间隔、汇率 Provider、主题、背景图、涨跌配色、美股盘前 / 盘后盈亏计入等）在后台“显示设置”中维护并持久化到数据库。美股盘前 / 盘后盈亏计入默认关闭；开关切换后会立即保存并刷新看板，开启后影响美股今日盈亏和实时历史点，不改变市场状态展示；若当前结算日已记录上一场美股常规收盘快照，且盘前阶段没有当天交易流水，则保留该快照，不用新一场盘前波动替换或冲减今日盈亏。A 股 / 港股在盘前、盘中、盘后阶段默认使用当日最新行情。
