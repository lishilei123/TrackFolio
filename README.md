...
- **后台设置**：单实例后台密码保护，支持验证码、错误锁定、显示设置、主题设置、背景图、涨跌配色、盘前 / 盘后盈亏计入开关、行情校验、历史重算、汇率刷新和后台密码修改。
...
已实现的核心能力包括：持仓和交易维护、后台密码保护、资产配置 JSON 导入导出、实时行情 / 基金净值刷新、多币种汇总、历史盈亏快照、主题与显示设置、盘前 / 盘后盈亏计入口径控制、移动端基础适配。
