...
- **盈亏看板**：展示总市值、今日盈亏、昨日盈亏、总持仓盈亏、历史曲线、持仓分布和盈亏贡献。
- **历史分析**：DailyPnL 每日快照落库，支持近 7 / 30 / 90 天、今年、自定义区间，以及日 / 周 / 月 / 年粒度聚合。
- **多币种折算**：支持 CNY / USD / HKD 统一结算货币；汇率可按 Provider 自动获取或手动刷新，汇率异常时给出缺口提示。
- **行情 Provider**：股票、基金行情通过可替换 Provider Adapter 接入，运行时按市场和资产类型自动选择数据源；美股盘前 / 盘后行情可独立开启，默认关闭。
- **后台设置**：单实例后台密码保护，支持验证码、错误锁定、显示设置、主题设置、背景图、涨跌配色、美股盘前 / 盘后行情开关、行情校验、历史重算、汇率刷新和后台密码修改。
...
已实现的核心能力包括：持仓和交易维护、后台密码保护、资产配置 JSON 导入导出、实时行情 / 基金净值刷新、多币种汇总、历史盈亏快照、主题与显示设置、美股盘前 / 盘后行情口径控制、移动端基础适配。
...
其他显示与数据源偏好（结算货币、结算时区、刷新间隔、汇率 Provider、主题、背景图、涨跌配色、美股盘前 / 盘后行情等）在后台“显示设置”中维护并持久化到数据库。美股盘前 / 盘后行情默认关闭；开启后只影响美股今日盈亏使用的最新价来源，不改变市场状态展示。
